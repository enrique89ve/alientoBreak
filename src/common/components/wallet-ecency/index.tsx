        ...
        try {
            if (activeUser) {
                ...
                if (response.status === 200) {
                    const userPoints: any[] = response.data.userPoints;
                    console.log(userPoints[0])
                    setUserPoints(userPoints[0])
                    return userPoints;
                } else if (response.status === 404) {
                    if (response.data.error === 'User not found') {
                        // Manejar error 'User not found'
                    } else if (response.data.error === 'Community not found for this user') {
                        // Manejar error 'Community not found for this user'
                    }
                }
            }
        } catch (error) {
            console.error('Error fetching user points:', error);
            // Considera manejar el error de una manera que sea útil para tu aplicación
            // Por ejemplo, podrías establecer un estado de error que se pueda mostrar en la interfaz de usuario
        }
        return undefined;
    };
