...
export class EntryListContent extends Component<Props, State> {
  ...
  fetchBlacklist = () => {
    getBlacklist().then((response) => {
      if (this._isMounted) {
        this.setState({ blacklist: response });
      }
    });
  };
    ...
    const dataToRender = entries.filter((entry) =>
      ...
      !entry.community
        ...
        (entry.community === global.hive_id ||
          (entry.json_metadata && Array.isArray(entry.json_metadata.tags) && entry.json_metadata.tags.some((tag) => global.tags.includes(tag))))
    );
